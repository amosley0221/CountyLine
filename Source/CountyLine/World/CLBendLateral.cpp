...
ACLBendLateral::ACLBendLateral()
{
    ...
    Sun->SetRelativeRotation(FRotator(-38,125,0));Sun->SetIntensity(3.15f);
    Sun->SetLightColor(FLinearColor(1.f,.94f,.84f));Sun->bAtmosphereSunLight=true;
    Sun->LightSourceAngle=1.25f;
    Sun->DynamicShadowDistanceMovableLight=18000.f;
    Sun->DynamicShadowCascades=3;
    Sun->ShadowCascadeBiasDistribution=.75f;
    ...
    Ambient->SetIntensity(1.05f);Ambient->SetRealTimeCaptureEnabled(true);
