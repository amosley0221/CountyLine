...
folder = '/Game/Art/Town/StreetFinishes'
projection = '''
float3 n = abs(N);
...
for name, (code, roughness) in patterns.items():
    ...
    # Small world-space relief catches grazing light without altering collision.
    if name in ('Brick','Wood','Stone','Siding'):
        height_code={
            'Brick': 'float2 c=float2(uv.x/25+fmod(abs(floor(uv.y/7.5)),2)*.5,uv.y/7.5); float2 e=min(frac(c),1-frac(c)); float h=.32*smoothstep(.02,.06,e.x)*smoothstep(.045,.10,e.y);',
            'Wood': 'float e=min(frac(uv.y/16),1-frac(uv.y/16)); float h=.12*smoothstep(.015,.06,e)+.015*sin(uv.x*.16+sin(uv.y*.75)*2);',
            'Stone': 'float2 c=float2(uv.x/65+fmod(abs(floor(uv.y/28)),2)*.5,uv.y/28);float2 e=min(frac(c),1-frac(c));float h=.16*smoothstep(.01,.04,e.x)*smoothstep(.015,.05,e.y);',
            'Siding': 'float h=.12*smoothstep(.01,.05,min(frac(uv.y/18),1-frac(uv.y/18)));'
        }[name]
        bump=lib.create_material_expression(mat,unreal.MaterialExpressionCustom)
        bump.set_editor_property('code',projection+height_code+'''
float3 wn=normalize(N);float3 dx=ddx(P),dy=ddy(P);
float3 rx=cross(dy,wn),ry=cross(wn,dx);float det=dot(dx,rx);
return normalize(max(abs(det),.000001)*wn-sign(det)*(ddx(h)*rx+ddy(h)*ry));
''')
        bump.set_editor_property('output_type',unreal.CustomMaterialOutputType.CMOT_FLOAT3)
        bump.set_editor_property('inputs',inputs)
        lib.connect_material_expressions(pos,'',bump,'P');lib.connect_material_expressions(normal,'',bump,'N')
...
unreal.log('CL_STREET_MATERIALS_SUCCESS')
